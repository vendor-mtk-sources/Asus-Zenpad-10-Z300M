...
#define MSDC_PAD_TUNE_CLKTDLY     (0x1f << 27)  /* CLKTDLY */
...
static void msdc_restore_reg(struct msdc_host *host)
{
        ...
        tmp = (host->save_para.pad_tune & ~MSDC_PAD_TUNE_CLKTDLY) | host->clkdelay;
        dev_dbg(host->dev, "tmp = %08x\n", tmp);
        writel(tmp, host->base + tune_reg);
	...
	if (check_data || (stop && stop->error)) {
		...
		if ((events & MSDC_INT_XFER_COMPL) && (!stop || !stop->error)) {
			...
			if (events & MSDC_INT_DATTMO)
			{
				...
				{
					dev_err(host->dev, "Enter %s, at %d, MSDC_CFG = 0x%x\n", __func__, __LINE__, readl(host->base + MSDC_CFG));
					regmap_update_bits(host->infracfg, 0x120, BIT(7), BIT(7));
					udelay(10);
					dev_err(host->dev, "Enter %s, at %d, MSDC_CFG = 0x%x\n", __func__, __LINE__, readl(host->base + MSDC_CFG));
					regmap_update_bits(host->infracfg, 0x124, BIT(7), BIT(7));
					regmap_update_bits(host->infracfg, 0x124, BIT(3), BIT(3));
					udelay(10);
					dev_err(host->dev, "Enter %s, at %d, MSDC_CFG = 0x%x\n", __func__, __LINE__, readl(host->base + MSDC_CFG));
				}
				msdc_restore_reg(host);
...
static ssize_t msdc_get_cd(struct mmc_host *mmc)
{
    struct device_node *np = mmc->parent->of_node;
    enum of_gpio_flags flags = OF_GPIO_ACTIVE_LOW;
    unsigned int pin = of_get_named_gpio_flags(np, "cd-gpios", 0, &flags);
    int status;
...
static int msdc_tune_response(struct mmc_host *mmc, u32 opcode)
{
	...
	sdr_set_bits(host->base + tune_reg, host->clkdelay);
	dev_dbg(host->dev, "Before tuning: MSDC_PAD_TUNE0 = %08x\n", readl(host->base + tune_reg));
...
skip_fall:
	...
	dev_dbg(host->dev, "After tuning: MSDC_PAD_TUNE0 = %08x\n", readl(host->base + tune_reg));
...
static int set_clk_delay(struct mmc_host *mmc, char * clktdelay_mode)
{
	struct msdc_host *host = mmc_priv(mmc);
	int index;

	index = of_property_match_string(host->dev->of_node, "clktdly_str", clktdelay_mode);
	if(index >= 0){
		dev_dbg(host->dev, "Tuning bus speed mode clkdelay : %s , param : %d\n", clktdelay_mode, host->clkdelay_arr[index]);
		host->clkdelay = (host->clkdelay_arr[index] << (ffs((unsigned int)MSDC_PAD_TUNE_CLKTDLY) - 1)) & MSDC_PAD_TUNE_CLKTDLY;
	}
...
static struct mmc_host_ops mt_msdc_ops = {
	...
	.clk_delay_tuning = set_clk_delay,
...
static ssize_t read_clk_delay(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct mmc_host *mmc = platform_get_drvdata(pdev);
	struct msdc_host *host;
	host = mmc_priv(mmc);
	return snprintf(buf, PAGE_SIZE, "host->base=0x%08x\n  MSDC_PAD_TUNE0=0x%08x\n",
			host->base , readl(host->base + MSDC_PAD_TUNE0));
}

static ssize_t write_clk_delay(struct device *dev, struct device_attribute *attr, char *buf, size_t count)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct mmc_host *mmc = platform_get_drvdata(pdev);
	struct msdc_host *host;
	...
	host = mmc_priv(mmc);
	kstrtol(buf, 10, &tmp);
	host->clkdelay = (tmp << (ffs((unsigned int)MSDC_PAD_TUNE_CLKTDLY) - 1)) & MSDC_PAD_TUNE_CLKTDLY;
	dev_dbg(&pdev->dev, "set clk_delay: %08x\n", host->clkdelay);
	return count;
}

static DEVICE_ATTR(tuneclk, 0644, read_clk_delay, write_clk_delay);
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (!of_property_read_u32(pdev->dev.of_node, "clktdly_num", &clktdly_mode_num)){
		host->clkdelay_arr = kmalloc(sizeof(u32)*clktdly_mode_num, GFP_KERNEL);
		if(!host->clkdelay_arr)
			dev_dbg(host->dev, "clkdelay_arr malloc fail!");
		else
			of_property_read_u32_array(pdev->dev.of_node, "clktdly", host->clkdelay_arr, clktdly_mode_num);
	}

	pr_info("%s: caps=0x%08x, caps2=0x%08x, pm_caps=0x%08x",
		mmc_hostname(host->mmc), host->mmc->caps, host->mmc->caps2, host->mmc->pm_caps);
