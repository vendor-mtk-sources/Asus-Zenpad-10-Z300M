...
static irqreturn_t mmc_gpio_cd_irqt(int irq, void *dev_id)
{
	...
	status = host->ops->get_cd(host);
	old_status = ctx->status;
	pr_info("%s: get card detect status : %d\n",mmc_hostname(host), status);

	/*  To be sure the card is really inserted or removed.
	 *  Floating will cause status change,
	 *  so reset 'retry' to 100 for getting more stable.
	 */
	if(status >= 0 && status ^ old_status){
		pr_info("%s: slot status change detected (%d -> %d)\n",
                                mmc_hostname(host), old_status, status);
		old_status = status;
	}
	else{
		pr_err("%s: SD slot status still %d (skip)\n",mmc_hostname(host) ,status);
		return IRQ_HANDLED;
	}

	while (status >= 0 && expired-- && retry--)
	{
		udelay(10);
		status = host->ops->get_cd(host);
		if (status >= 0 && status ^ old_status){
			pr_debug("%s: slot status floating detected (%d -> %d)\n",
                                mmc_hostname(host), old_status, status);
			retry = 100;
			old_status = status;
		}
...
void mmc_gpiod_request_cd_irq(struct mmc_host *host)
{
	...
	ctx->status = host->ops->get_cd(host);
