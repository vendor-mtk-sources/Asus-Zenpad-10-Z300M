...
static bool screen_status = false;

static slimport_fb_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if( event == FB_EVENT_BLANK){
        ...
        if(*blank == FB_BLANK_UNBLANK){
			screen_status = 1; //screen on
        }else if( *blank == FB_BLANK_POWERDOWN){
			screen_status = 0; //screen off
		}
	}
...
static struct notifier_block slimport_fb_notifier = {
	 .notifier_call = slimport_fb_notifier_callback,
};
...
struct anx7816_data {
	...
	// add mechanism to wakeup screen
...
static struct device_attribute slimport_device_attrs[] = {
	...
	__ATTR(SlimPort_status, S_IRUGO, show_i2c_status, NULL),
#ifdef SP_REGISTER_SET_TEST	/*//slimport test*/
...
static void anx7816_free_gpio(struct anx7816_data *anx7816)
{
	...
	gpio_free(anx7816->pdata->gpio_i2c_ctrl);
}
...
static int anx7816_init_gpio(struct anx7816_data *anx7816)
{
	...
	/* gpio for slimport i2c power */
	ret = gpio_request(anx7816->pdata->gpio_i2c_ctrl,
				"anx7816_i2c_ctrl");
	if (ret) {
		pr_err("%s : failed to request gpio %d\n",
		       __func__, anx7816->pdata->gpio_i2c_ctrl);
		goto err5;
	}
	gpio_direction_output(anx7816->pdata->gpio_i2c_ctrl, 0);
	gpio_set_value(anx7816->pdata->gpio_i2c_ctrl,1);
...
static void wake_up_screen(void *data)
{
	struct anx7816_data *td = data;
	if(!screen_status){
		printk("Slimport in Wake up Screen\n");
		input_report_key(td->input_dev,KEY_POWER,1);
		input_sync(td->input_dev);
		input_report_key(td->input_dev,KEY_POWER,0);
		input_sync(td->input_dev);
	}
}
...
static irqreturn_t anx7816_cbl_det_isr(int irq, void *data)
{
	...
	if (cable_connected == 0x01) {
		...
		queue_delayed_work(anx7816->workqueue, &anx7816->work, msecs_to_jiffies(10));
		wake_up_screen(anx7816);
		irq_status = true;
	} else {
			    ...
			    struct anx7816_platform_data *pdata)
{
	...
	pdata->gpio_i2c_ctrl =
		of_get_named_gpio_flags(np, "analogix,i2c-ctrl-gpio", 0,
					NULL);

	pr_info("%s gpio_i2c_ctrl %d",LOG_TAG, pdata->gpio_i2c_ctrl);
			     ...
			     const struct i2c_device_id *id)
{
	...
	anx7816->input_dev = input_allocate_device();
	anx7816->input_dev->name = "anx7816";

	ret = input_register_device(anx7816->input_dev);

	if(ret){
		printk("%s: unable register slimport input device\n",__func__);
	}

	input_set_capability(anx7816->input_dev, EV_KEY, KEY_POWER);

	if(fb_register_client(&slimport_fb_notifier)){
