...
static irqreturn_t pwrkey_int_handler(int irq, void *dev_id)
{
	...
	ktime_t ktime;
        ...
        static unsigned long timer_bef;
	static unsigned int bat_volt;

	if (upmu_get_pwrkey_deb() == 1) {
		pr_notice("[pwrkey_int_handler] Release pwrkey\n");
                timer_cur = sched_clock();
                /* Detect long press power key (1s) */
                if (timer_cur - timer_bef >= 1000000000)
                     pr_notice("Long Press Power Key - %ld(s)\n",(timer_cur - timer_bef)/1000000000);
#if defined(CONFIG_MTK_KERNEL_POWER_OFF_CHARGING)
		if (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0) {
			...
			bat_volt = battery_meter_get_battery_voltage(KAL_TRUE);
			if (long_pwrkey_press &&
				BMT_status.UI_SOC >= 1 &&
				bat_volt > 3500) {	/* 500ms; UI_SOC >= 1%; VBAT > 3.5V*/
				pr_notice
				    ("Power Key Pressed during kernel power off charging, reboot OS(%d)(%d)\r\n",bat_volt,BMT_status.UI_SOC);
				arch_reset(0, NULL);
			}else{
				long_pwrkey_press = false;
			}
		}
#endif
...
static irqreturn_t chrdet_int_handler(int irq, void *dev_id)
{
	...
	if (!upmu_get_rgs_chrdet()) {
		...
		if ((boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		    || boot_mode == LOW_POWER_OFF_CHARGING_BOOT) && (b_COS_CO == false)) {
			do {
				pr_notice("Unplug Charger/USB In Kernel Power Off Charging Mode!  Shutdown OS!\r\n");
				msleep(200);
                        } while (chargin_hw_init_done == 0);
			#ifdef CONFIG_MTK_RTC
			bq25896_set_en_ilim(0x1);       // enable current limit
			mt_power_off();
			#endif
		}
	}
...
#ifndef USER_BUILD
extern void current_wakeup_sources_show(void);
#endif
static irqreturn_t fchr_key_int_handler(int irq, void *dev_id)
{
	if (upmu_get_fchrkey_deb() == 1) {
		...
		upmu_set_rg_fchrkey_int_sel(0);
                /*Set release volumn down key to show kernel wakelock message */
#ifndef USER_BUILD
                current_wakeup_sources_show();
#endif
	} else {
...
static int mt6323_irq_handler_init(struct mt6323_chip_priv *chip)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6323_irqs); i++) {
		...
		if(data->irq_id == RG_INT_STATUS_CHRDET)
			g_vcdt_irq = irq;
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	if (chip_version >= PMIC6323_E2_CID_CODE) {
		...
		ret = pmic_config_interface(0x2, 0x0, 0xF, 0);  /* [3:1]: RG_VCDT_LV_VTH: 4.2V Charging Detect*/
...
static int pmic_mt6323_probe(struct platform_device *dev)
{
	...
	device_create_file(&(dev->dev), &dev_attr_pmic_access);
        ...
        upmu_set_rg_chrwdt_en(0);                       //Disable charger watchdog timer, 0x1A, 4bit
        upmu_set_rg_chrwdt_int_en(0);                   //Disable Interrupt setting for charger watchdog timer, 0x1E, 0bit
        upmu_set_rg_chrwdt_wr(0);                       //Reset inactive for charger watchdog timer, 0x1A, 8bit
        upmu_set_rg_chrwdt_flag_wr(1);                  //Clear time-out flag for charger watchdog timer, 0x1E, 1bit
