...
//	}
	...
	for(index = 0 ; index < AP3425H_TABLE_LENGTH ; index++) {
		if( value <= defalut_table[index] ) {
			ap3425_set_ahthres(ap3xx6_obj->client, cali_table[index] );
			ap3425_set_althres(ap3xx6_obj->client, (index ==0) ? 0 : (cali_table[index-1]+1)  );
			break;
		}
	}
	enable_irq(alsps_irq);
...
//	struct ap3xx6_priv *obj = ap3xx6_obj;
	...
	ap3xx6_read_als(ap3xx6_obj->client, &ap3xx6_obj->als);
	if(ap3xx6_openlog) APS_LOG("ap3xx6 ALS level=%d\n", ap3xx6_obj->als);
	...
	if (ap3xx6_obj->als == 0) {
		...
		(b[1] > b[0])?(ap3xx6_obj->als = b[0]):(ap3xx6_obj->als = b[1]);
		temp_value = ap3xx6_obj->als;//ap3xx6_get_als_value(ap3xx6_obj, ap3xx6_obj->als);
		temp_als = temp_value;
	}
