...
int fts_ts_resume(struct device *dev)
{
	...
	/* Check is MOS */
	if(entry_mode != 1) {
		LDBG("In COS Mode , Bypass Resume , entry_mode [%d]\n", entry_mode);
		return 0;
	}
...
static int ft5726_reload_test_ini(void)
{
	...
	if(!gt5726_test_ini)
	{
		LDBG("allocate gt5726_test_ini memory fail !\n");
		return -EIO;
	}
...
static ssize_t touch_switch_name(struct switch_dev *sdev, char *buf)
{
	...
	{
		case BIEL_OGS_BLACK:
			return sprintf(buf, "BIEL_OGS_BLACK_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		case BIEL_GFF_WHITE:
			return sprintf(buf, "BIEL_GFF_WHITE_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		case BIEL_OGS_WHITE:
			return sprintf(buf, "BIEL_OGS_WHITE_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		case LAIB_OGS_BLACK:
			return sprintf(buf, "LAIB_OGS_BLACK_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		default:
			return sprintf(buf, "FOCAL_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
	}
}
...
*/
	...
	*/
	...
	if(build_version == 1){
		...
		{
			case BIEL_OGS_BLACK:
				if(data->fw_ver[0] < 0x49) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[BIEL_OGS_BLACK] force update to version: [0x%2d] -> [0x49]\n", data->fw_ver[0]);
				} else {
			...
			case BIEL_OGS_WHITE:
				if(data->fw_ver[0] < 0x49) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[BIEL_OGS_WHITE] force update to version: [0x%2d] -> [0x49]\n", data->fw_ver[0]);
				} else {
					LDBG("[BIEL_OGS_WHITE][V%2d] check version pass, skip force FW update\n", data->fw_ver[0]);
				}
				break;
			case LAIB_OGS_BLACK:
				if(data->fw_ver[0] < 0x47) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[LAIB_OGS_BLACK] force update to version: [0x%2d] -> [0x47]\n", data->fw_ver[0]);
				} else {
...
//add by leo --
	...
	/* Check is MOS */
	if(entry_mode != 1) {
		LDBG("In COS Mode , Enter Suspend Mode , entry_mode [%d]\n",entry_mode);
		fts_ts_suspend(&client->dev);
		return 0;
	}
