...
// add by leo ++
#define DEFAULT_CHIP_ID_INDEX 12

int fts_ctpm_fw_download_with_i_file(struct i2c_client *client, char *firmware_name);
...
static unsigned char aucFW_PRAM_BOOT[] = {
	#include "FT5822_Pramboot_V0.3_20141015.i"
};
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
	...
	*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			LDBG(" Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
			continue;
		}
	}
...
void fts_get_upgrade_array(void)
{
	...
	{
		memcpy(&fts_updateinfo_curr, &fts_updateinfo[DEFAULT_CHIP_ID_INDEX], sizeof(struct fts_Upgrade_Info));
	}
}
...
int fts_6x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			LDBG(" Step 3: GET CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
		}
	}
	if (i >= FTS_UPGRADE_LOOP)
	...
	if (reg_val[0] != bt_ecc) 
	{
		LDBG("--ecc error! FW=%02x bt_ecc=%02x\n",
					reg_val[0],
					bt_ecc);
		return -EIO;
	}
...
int fts_6x06_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			LDBG(" Step 3: CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
		}
	}
	if (i > FTS_UPGRADE_LOOP)
		return -EIO;
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
              ...
              {
                      LDBG(" Step 3: CTPM ID FAILD,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
                      continue;
              }
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	LDBG("erase app area reg_val[0] = %x reg_val[1] = %x \n", reg_val[0], reg_val[1]);
	...
	for (j = 0; j < packet_number; j++) 
	{
		...
		if (bt_ecc != bt_ecc_check)
			LDBG("Host checksum error bt_ecc_check = %x \n", bt_ecc_check);
		fts_i2c_write(client, packet_buf, FTS_PACKET_LENGTH + 6);
		for (i = 0; i < 30; i++) 
		{
			auc_i2c_write_buf[0] = 0x6a;
	...
	for (i = 0; i < 100; i++) 
	{
		...
		reg_val[0] = reg_val[1] = 0x00;
		fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
		LDBG("--reg_val[0]=%02x reg_val[0]=%02x\n", reg_val[0], reg_val[1]);
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	LDBG("START ... fwsize: %d\n",fwsize); // add by leo for testtest
	if (fwsize <= 0) 
	{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
		...
		if (i_ret != 0)
		{
			LDBG("fts_8606_writepram failed. err.\n");
			return -EIO;
		}	
		LDBG("fts_8606_ctpm_fw_upgrade\n"); // add by leo for testtest
		i_ret =  fts_8606_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	if (i_ret != 0) {
		LDBG("ERROR: upgrade failed..\n");
...
int  fts_ctpm_fw_preupgrade_hwreset(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
		LDBG("ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]); // add by leo for testtest

		if (reg_val[0] == 0x58
			&& reg_val[1] == 0x22) 
		{
			fts_read_reg(client, 0xd0, &ret);

			if(ret == 0)
			{
				LDBG("Step 3: READ State fail \n");
				continue;
			}
			...
			LDBG("Step 3: READ CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
	...
	LDBG("checksum %X %X \n",reg_val[0],bt_ecc);
	LDBG("Read flash and compare\n");
		
	mdelay(50);

	/*********Step 6: start app***********************/
	LDBG("Step 6: start app\n");
	auc_i2c_write_buf[0] = 0x08;
	fts_i2c_write(client, auc_i2c_write_buf, 1);
	mdelay(20);
...
int fts_ctpm_fw_download(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		fts_write_reg(client, 0xfc, FTS_UPGRADE_AA);
		mdelay(fts_updateinfo_curr.delay_aa);
		...
		if (i<=15)
		{
			mdelay(fts_updateinfo_curr.delay_55+i*3);
		}
		else
		{
			mdelay(fts_updateinfo_curr.delay_55-(i-15)*2);
		}

		LDBG("Download Step 2:Enter upgrade mode \n");
		/*********Step 2:Enter upgrade mode *****/
		...
		/*********Step 3:check READ-ID***********************/
		mdelay(fts_updateinfo_curr.delay_readid);
		auc_i2c_write_buf[0] = 0x90;
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] =0x00;
		fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
		...
		if (reg_val[0] == 0x58
		        && reg_val[1] == 0x2b)
		{
			LDBG("Download Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
			    reg_val[0], reg_val[1]);
			break;
		}
		else
	...
	LDBG("Step zaxzaxzaxzax Download Step change to write flash mode\n");
	if (i >= FTS_UPGRADE_LOOP)
	{LDBG("Step z11111axzaxzaxzax Download Step change to write flash mode\n");
		if (reg_val[0] != 0x00 || reg_val[1] != 0x00)
		{LDBG("Step zax333333zaxzaxzax Download Step change to write flash mode\n");
			if (gpio_is_valid(private_ts->pdata->reset_gpio)) {
				gpio_set_value_cansleep(private_ts->pdata->reset_gpio, 0);
				mdelay(10);
				gpio_set_value_cansleep(private_ts->pdata->reset_gpio, 1);
		...
		LDBG("Step zaxza222222xzaxzax Download Step change to write flash mode\n");
		return -EIO;
	}
 
	LDBG("Download Step 4:change to write flash mode\n");
	fts_write_reg(client, 0x09, 0x0a);

	/*Step 4:erase app and panel paramenter area*/
	LDBG("Download Step 4:erase app and panel paramenter area\n");
	auc_i2c_write_buf[0] = 0x61;
	fts_i2c_write(client, auc_i2c_write_buf, 1);	/*erase app area */
	mdelay(fts_updateinfo_curr.delay_erase_flash);
	/*erase panel parameter area */
	...
	LDBG("Download Step 5:write firmware(FW) to ctpm flash\n");
	/*********Step 5:write firmware(FW) to ctpm flash*********/
	bt_ecc = 0;
	...
	LDBG("Download Step 6: read out checksum\n");
	/*********Step 6: read out checksum***********************/
	auc_i2c_write_buf[0] = 0x64;
	fts_i2c_write(client, auc_i2c_write_buf, 1);
	mdelay(300);
	...
	if (reg_val[0] != bt_ecc)
	{
		LDBG(" Download Step 6: ecc error! FW=%02x bt_ecc=%02x\n",
		        reg_val[0],
		        bt_ecc);
	...
	fts_ctpm_hw_reset();
#endif
	mdelay(300);	/*make sure CTP startup normally */

	i_ret = hidi2c_to_stdi2c(client);//Android to Std i2c.

	if(i_ret == 0)
	{
		LDBG("HidI2c change to StdI2c i_ret = %d ! \n", i_ret);
	}
...
int fts_ctpm_fw_download_with_i_file(struct i2c_client *client, char *firmware_name)
{
	...
	if (fts_ReadFirmware(firmware_name, pbt_buf)) 
	{
		...
		if (i_ret != 0)
		{
			LDBG(" Step FTS write pram failed. err.\n");
			
			return -EIO;
		}
		
		/*call the upgrade function */
		//i_ret = fts_ctpm_fw_download(client, aucFW_ALL_AUO, sizeof(aucFW_ALL_AUO));
		i_ret = fts_ctpm_fw_download(client, pbt_buf, fwsize);
		if (i_ret != 0)
		{
			LDBG(" Step FTS upgrade failed. err.\n");
			return -EIO;
		}
		
	
	LDBG("\n Step FTS zax  i_ret=%d\n",i_ret);
	return i_ret;
}
// add by leo --
