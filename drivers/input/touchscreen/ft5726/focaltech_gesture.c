...
*******************************************************************************/
#define  KEY_GESTURE_DOUBLE_CLICK	KEY_F24
#define  KEY_GESTURE_UP				KEY_UP
#define  KEY_GESTURE_DOWN			KEY_DOWN
#define  KEY_GESTURE_LEFT			KEY_LEFT
#define  KEY_GESTURE_RIGHT			KEY_RIGHT
#define  KEY_GESTURE_O				KEY_O
#define  KEY_GESTURE_E				KEY_F21
#define  KEY_GESTURE_C				KEY_F20
#define  KEY_GESTURE_M				KEY_M
#define  KEY_GESTURE_L				KEY_L
#define  KEY_GESTURE_W				KEY_F23
#define  KEY_GESTURE_S				KEY_F22
#define  KEY_GESTURE_V				KEY_F18
#define  KEY_GESTURE_Z				KEY_F19
...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	...
	printk("[ft5726] %s: gesture_id = 0x%02X\n", __func__, gesture_id);

	switch(gesture_id)
	{
		case GESTURE_LEFT:
			printk("[ft5726] %s: GESTURE_LEFT\n", __func__);
		...
		case GESTURE_DOUBLECLICK:
			if(private_ts->gesture & _GESTURE_DOUBLE_CLICK)
			{
				printk("[ft5726] %s: GESTURE_DOUBLECLICK\n", __func__);
				input_report_key(input_dev, KEY_GESTURE_DOUBLE_CLICK, 1);
				input_sync(input_dev);
				input_report_key(input_dev, KEY_GESTURE_DOUBLE_CLICK, 0);
				input_sync(input_dev);
			} else {
				printk("[ft5726] %s: SKIP GESTURE_DOUBLECLICK ..\n", __func__);
			}
			break;
		case GESTURE_O:
			printk("[ft5726] %s: GESTURE_O\n", __func__);
			...
			break;
		case GESTURE_W:
			if(private_ts->gesture & _GESTURE_W)
			{
				printk("[ft5726] %s: GESTURE_W\n", __func__);
				input_report_key(input_dev, KEY_GESTURE_W, 1);
				input_sync(input_dev);
				input_report_key(input_dev, KEY_GESTURE_W, 0);
				input_sync(input_dev);
			} else {
				printk("[ft5726] %s: SKIP GESTURE_W ..\n", __func__);
			}
			break;
		case GESTURE_M:
			printk("[ft5726] %s: GESTURE_M\n", __func__);
			...
			break;
		case GESTURE_E:
			if(private_ts->gesture & _GESTURE_E)
			{
				printk("[ft5726] %s: GESTURE_E\n", __func__);
				input_report_key(input_dev, KEY_GESTURE_E, 1);
				input_sync(input_dev);
				input_report_key(input_dev, KEY_GESTURE_E, 0);
				input_sync(input_dev);
			} else {
				printk("[ft5726] %s: SKIP GESTURE_E ..\n", __func__);
			}
			break;
		case GESTURE_C:
			if(private_ts->gesture & _GESTURE_C)
			{
				printk("[ft5726] %s: GESTURE_C\n", __func__);
				input_report_key(input_dev, KEY_GESTURE_C, 1);
				input_sync(input_dev);
				input_report_key(input_dev, KEY_GESTURE_C, 0);
				input_sync(input_dev);
			} else {
				printk("[ft5726] %s: SKIP GESTURE_C ..\n", __func__);
			}
			break;
		case GESTURE_L:
			printk("[ft5726] %s: GESTURE_L\n", __func__);
			...
			break;
		case GESTURE_S:
			if(private_ts->gesture & _GESTURE_S)
			{
				printk("[ft5726] %s: GESTURE_S\n", __func__);
				input_report_key(input_dev, KEY_GESTURE_S, 1);
				input_sync(input_dev);
				input_report_key(input_dev, KEY_GESTURE_S, 0);
				input_sync(input_dev);
			} else {
				printk("[ft5726] %s: SKIP GESTURE_S ..\n", __func__);
			}
			break;
		case GESTURE_V:
			if(private_ts->gesture & _GESTURE_V)
			{
				printk("[ft5726] %s: GESTURE_V\n", __func__);
				input_report_key(input_dev, KEY_GESTURE_V, 1);
				input_sync(input_dev);
				input_report_key(input_dev, KEY_GESTURE_V, 0);
				input_sync(input_dev);
			} else {
				printk("[ft5726] %s: SKIP GESTURE_V ..\n", __func__);
			}
			break;
		case GESTURE_Z:
			if(private_ts->gesture & _GESTURE_Z)
			{
				printk("[ft5726] %s: GESTURE_Z\n", __func__);
				input_report_key(input_dev, KEY_GESTURE_Z, 1);
				input_sync(input_dev);
				input_report_key(input_dev, KEY_GESTURE_Z, 0);
				input_sync(input_dev);
			} else {
				printk("[ft5726] %s: SKIP GESTURE_Z ..\n", __func__);
			}
			break;
	        default:
	                break;
