...
{
    return gpio_get_value(COVER_ATTACH_GPIO) ? true : false ; //false : true;
}
...
//            else if (entry_mode == 4 && (cos_keep_power_on_with_cb81 || charging_status == POWER_SUPPLY_STATUS_CHARGING || charging_status == POWER_SUPPLY_STATUS_FULL))
            else if (entry_mode == 4 && (charging_status == POWER_SUPPLY_STATUS_CHARGING || charging_status == POWER_SUPPLY_STATUS_FULL))
            {
                BAT_DBG("cos_keep_power_on_with_cb81: null, charging_status: %d\n",  charging_status);
                val->intval = 1;
            }
            else
                val->intval = 0;
...
#if defined(CONFIG_Z380KL) || defined(CONFIG_Z380C)||defined(CONFIG_Z380M)
...
int smb3xxc_jeita_control(int bat_tempr, int bat_volt, bool exec_jeita)
{
    ...
    int Vflt_IV = 0;
    int Volt_IV = 0;
    int Fast_Current_High = 0;
    int Fast_Current_Low = 0;
...
int smb3xxc_pre_config(bool initial, bool cover_changed_cable_changed)
{
    ...
    /* set Float Voltage: 4.35V. 03h[5:0]="101011" */
    if (cover_changed_cable_changed)
    ret = smb345c_masked_write(smb345c_dev->client,
            0x03,
            BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0),
...
void cover_aicl(int I_USB_IN)
{
    ...
    if (I_USB_IN == 500)
        ...
        reg_val = 0x00;
    else if (I_USB_IN == 1000)
        reg_val = 0x30;
    else if (I_USB_IN == 1500)
        reg_val = 0x50;
    else
        reg_val = 0x10;
pr_err("[%s] reg_val = %d\n", __func__, reg_val);
    ...
    if (smb345c_masked_write(smb345c_dev->client, 0x02, BIT(4), 0x1)) {
        dev_err(&smb345c_dev->client->dev,
