...
#if defined(CONFIG_Z380C) || defined(CONFIG_Z380KL) || defined(CONFIG_Z380M)
...
GBOOL _I2cExeCmd(GI2CCmdType *pI2CCmd)
{
  ...
  if(pI2CCmd->bRdCnt > 0)
  {
    ...
    msg_size = 2;
    #if defined(CONFIG_Z380C)||defined(CONFIG_Z380M) 
  ...
  if(wSysMode & SYS_MODE_INIT_CAP)
  {
    ...
    else
    {
      ...
      if(upi_ug31xx->cable_change_cnt)
      {
        ...
        cancel_delayed_work(&upi_ug31xx->power_change_work);
...
{
   GLOGD("[%s]\n", __func__);
  cancel_delayed_work(&upi_ug31xx->info_update_work);
...
//   gauge_attached = COVER_ATTACHED_UPI() ?1:0;
...
#endif
{
  GLOGE("<BATT>[%s]: uG31xx attached? %s.\n", __func__, attach? "true":"false" );
  cancel_delayed_work(&upi_ug31xx->power_change_work);
  cancel_delayed_work(&upi_ug31xx->info_update_work);

  GLOGE("<BATT>[%s]:cancel_delayed_work_sync\n", __func__);
...
void upi_ug31xx_alarm_callback(void)
{
  GWORD wAlarmSts;
...
{
  cancel_delayed_work(&upi_ug31xx->kbo_data.work);
  ...
  switch (psp)
  {
    ...
    case POWER_SUPPLY_PROP_STATUS:
      ...
      if(gauge_attached == 1)
      {
        #if defined(CONFIG_Z380C) || defined(CONFIG_Z380KL)||defined(CONFIG_Z380M)
        SmbReadCommand(SBS_RSOC);
        #else   ///< else of defined(CONFIG_Z380C) || defined(CONFIG_Z380KL)||defined(CONFIG_Z380M)
        SmbReadCommand(SBS_BS);
        #endif  ///< end of defined(CONFIG_Z380C) || defined(CONFIG_Z380KL)||defined(CONFIG_Z380M)
        wData = (GWORD)(*(SBSCmd->pbData + 1));
        wData = (wData << 8) | (*SBSCmd->pbData);
      }
      #else   ///< else of UG31XX_ATTACH_DETECTION
...
//        val->intval = smb345c_get_charging_status();
          if((VBUS_IN() && !g_sdp_cb81_padcharging)||BMT_status.charger_type == STANDARD_CHARGER)
