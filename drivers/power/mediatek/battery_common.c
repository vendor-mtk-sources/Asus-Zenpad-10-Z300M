 ...
 * Alarm Timer Related
 */
static struct alarm Wake_BAT_alarm_timer;	//alarm timer
static atomic_t Wake_BAT_alarm;			//alarm trigger set 1
static struct workqueue_struct *atimer_wq;	//alarm wq
static struct delayed_work atimer_work;
...
#endif
static int g_bootup_skip_count = 1;//for check soc_update count at start
static int g_boot_up_first = 1;//for check first boot to change UI_SOC
bool b_COS_CO = false;
bool is_full_charging = false;
signed long BAT_TASK_PERIOD = 10;
struct wake_lock battery_COS_CO_lock;
...
unsigned int _g_bat_sleep_short_total_time = NORMAL_WAKEUP_PERIOD;
int g_smartbook_update = 0;
int g_battery_pmevent = 0;
...
kal_bool chr_wake_up_for_meter_check = KAL_FALSE;
kal_bool chr_wake_up_for_meter_charging = KAL_TRUE;
kal_bool chr_wake_up_for_meter_discharging = KAL_TRUE;
...
/*
 *Global Function for check cable / vbus / cover
*/

//int (*touch_handler)(int state);
struct delayed_work firstbat_work;
static struct workqueue_struct *firstbat_wq;
...
//		val->intval = data->BAT_batt_vol;
		/*Pressing Power-Key will check this file node for reboot in AC-COS mode.*/
		/*Change from average voltage to real battery voltage.*/
...
static ssize_t show_COS_CO(struct device *dev, struct device_attribute *attr,
                                        char *buf)
{
	battery_log(BAT_LOG_CRTI, "[%s] COS_CO = %d\n",__func__, b_COS_CO);
        return sprintf(buf, "%d\n", b_COS_CO);
}

static ssize_t store_COS_CO(struct device *dev, struct device_attribute *attr,
...
static DEVICE_ATTR(COS_CO, 0664, show_COS_CO, store_COS_CO);
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE) {	/* charging full first, UI tracking to 100% */
		...
		battery_log(BAT_LOG_CRTI, "[100percent]UI_SOC(%d),g_bootup_skip_count(%d),reset(%d)\n",
			    BMT_status.UI_SOC, g_bootup_skip_count, resetBatteryMeter);
	} else {
		/* charging is not full,  UI keep 99% if reaching 100%, */
		/* unless Charger IC might recognize wrong full charging type after boot up, */
		/* just skip 3 times at the begining */
		if (BMT_status.UI_SOC >= 99 ) {
			if(g_bootup_skip_count <= 3 && BMT_status.UI_SOC == 100){
				resetBatteryMeter = KAL_FALSE;
			}else{
		...
		battery_log(BAT_LOG_CRTI, "[100percent]UI_SOC(%d),g_bootup_skip_count(%d),resetBatteryMeter(%d)\n",
					BMT_status.UI_SOC,
					g_bootup_skip_count,
					resetBatteryMeter);
		timer_counter = (cust_sync_time / BAT_TASK_PERIOD);
	}
	if(g_bootup_skip_count <= 3)
		g_bootup_skip_count++;
	return resetBatteryMeter;
}
...
{
	static unsigned int timer_counter;
	static unsigned int bq25896_ICharging;
	battery_charging_control(CHARGING_CMD_GET_bq25896_ICharging,&bq25896_ICharging);
	battery_log(BAT_LOG_CRTI,"[%s] bq25896_ICharging = %d",__func__,bq25896_ICharging);

	if ((BMT_status.UI_SOC > BMT_status.SOC) && (BMT_status.UI_SOC != 1)) {
#if !defined(SYNC_UI_SOC_IMM)
		/* reduce after xxs */
		/*Charger IC current = 0;
		**No interrupt => chr_wake_up_for_meter_check == 0*/
		if (chr_wake_up_bat == KAL_FALSE && bq25896_ICharging == 0 &&
			chr_wake_up_for_meter_check == 0) {
			if (timer_counter ==
			    (batt_cust_data.sync_to_real_tracking_time / BAT_TASK_PERIOD)) {
			//Discharging Need Buffer, 2mins, 1min30secs, 1min10secs
				BMT_status.UI_SOC--;
				timer_counter = 0;
			}
#ifdef FG_BAT_INT
			...
			if(chr_wake_up_for_meter_check == 1){
				battery_log(BAT_LOG_CRTI,"[%s] Cable Out Interrupt!!! Do not update UI_SOC.\n",__func__);
			}
		}
#else
		BMT_status.UI_SOC--;
		...
		if(g_boot_up_first){
			...
			if ((bq25896_ICharging > 0) && (BMT_status.UI_SOC < BMT_status.SOC) &&
				(chr_wake_up_for_meter_check == 0)){
			...
			if (chr_wake_up_for_meter_check == 1){
				battery_log(BAT_LOG_CRTI,"[%s] Cable In Interrupt!!! Do not update UI_SOC.\n",__func__);
			}
		}
#else
...
{
	static unsigned int bq25896_ICharging;
	static unsigned int bq25896_VREG;
	static unsigned int VBAT;
	...
	if (BMT_status.SOC > 60 && b_eng_charging_limit == true){//the condition of stop charging
		battery_log(BAT_LOG_CRTI, "[%s] the condition of stop charging \n", __func__);
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_NOT_CHARGING;
	}
#endif
#if defined(CONFIG_Z380M)
	if (BMT_status.old_charging_mode == COVER_SDP ){//the condition of stop charging
		if(g_sdp_cb81_padcharging && upmu_is_chr_det() == KAL_TRUE){
			battery_log(BAT_LOG_FULL, "[%s] g_sdp_cb81_padcharging = %d,upmu_is_chr_det = %d\n", __func__,
							 g_sdp_cb81_padcharging,upmu_is_chr_det());
			bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_CHARGING;
		}
		else{
			battery_log(BAT_LOG_FULL, "[%s] g_sdp_cb81_padcharging = %d,upmu_is_chr_det = %d\n", __func__,
...
#endif
	/*check oam_i_2 & charging state is correct or not**
	**Negative oam_i_2 == charging 			  **
	**Positive oam_i_2 == discharging		  */
	battery_charging_control(CHARGING_CMD_GET_bq25896_ICharging,&bq25896_ICharging);
	battery_charging_control(CHARGING_CMD_GET_VREG,&bq25896_VREG);
	VBAT = battery_meter_get_battery_voltage(KAL_TRUE);
	battery_log(BAT_LOG_FULL, "[%s] VBAT = %d, VREG = %d\n",__func__,VBAT, bq25896_VREG);
	if(VBAT < bq25896_VREG && g_boot_up_first == 0){
		if((oam_i_2 > 0 && bq25896_ICharging > 0) || (oam_i_2 < 0 && bq25896_ICharging == 0)){
			battery_log(BAT_LOG_CRTI, "[%s]MeterReset!!(%d) oam_i_2 = %d, ICHG = %d\n",__func__, BMT_status.UI_SOC, oam_i_2, bq25896_ICharging);
			resetBatteryMeter = KAL_TRUE;
		}
	}
	...
	if (resetBatteryMeter == KAL_TRUE) {
		...
		if (bat_is_recharging_phase() == KAL_TRUE) {
			if(BMT_status.UI_SOC == 100){
				...
				battery_meter_reset();
				battery_log(BAT_LOG_CRTI, "[%s]Recharging, meter_reset(%d)\n",__func__,BMT_status.UI_SOC);
			}
...
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if ((BMT_status.charger_type == STAND_AC) ||
...
static void mt_battery_average_method_init(BATTERY_AVG_ENUM type, unsigned int *bufferdata,
					   unsigned int data, signed int *sum, signed int sw)
{
	unsigned int i;
	...
	/*Fix average voltage not update*/
	index++;
        if (index >= 4) {
                index = 4;
					    ...
					    unsigned char batteryIndex)
{
	...
	mt_battery_average_method_init(type, bufferdata, data, sum, 0);
...
static unsigned int mt_battery_average10_method(BATTERY_AVG_ENUM type, unsigned int *bufferdata,
                                            unsigned int data, signed int *sum,
                                            unsigned char batteryIndex)
{
        unsigned int avgdata;

        mt_battery_average_method_init(type, bufferdata, data, sum, 10);
...
{
#if defined(CONFIG_Z380M)
	unsigned int Avg10_bat_vol, bat_vol, charger_vol, Vsense, ZCV, pack_vol;
#else
	unsigned int Avg10_bat_vol, bat_vol, charger_vol, Vsense, ZCV;
#endif
	signed int ICharging, temperature, temperatureR, temperatureV, SOC;
	static signed int bat10_sum, bat_sum, icharging_sum, temperature_sum;
	static signed int batteryVoltageBuffer[BATTERY_AVERAGE_SIZE];
	static signed int batteryVoltageBuffer10[10];
	...
	static unsigned char battery10Index;
	...
	//bat_meter_timeout		->routine thread. bat_thread_wakeup
	//bat_spm_timeout		->sleep time > 5400secs.
	//fg_wake_up_bat		->no need. wake_up_bat2
	//_g_bat_sleep_short_total_time	->AC-Charging sleep time > 60secs.
	if (bat_meter_timeout == KAL_TRUE || bat_spm_timeout == TRUE || fg_wake_up_bat == KAL_TRUE || _g_bat_sleep_short_total_time >= 60) {
		battery_charging_control(CHARGING_CMD_GET_CHARGING_STATUS,&is_full_charging);
		SOC = battery_meter_get_battery_percentage();

		if(BMT_status.charger_exist == KAL_TRUE){//Charging
			if(is_full_charging){//bat_full until Cable Out
				BMT_status.bat_charging_state = CHR_BATFULL;
		...
		battery_log(BAT_LOG_CRTI,"[%s] is_full_charging = %d, UI_SOC = %d, SOC = %d\n",__func__,is_full_charging,BMT_status.UI_SOC,SOC);
		bat_meter_timeout = KAL_FALSE;
		bat_spm_timeout = FALSE;
		_g_bat_sleep_short_total_time = 0;
	} else {
		if (previous_SOC == -1)
			SOC = battery_meter_get_battery_percentage();
		else
	...
	if (previous_SOC == -1 && bat_vol <= batt_cust_data.v_0percent_tracking) {
		...
		Avg10_bat_vol =
                    mt_battery_average10_method(BATTERY_AVG_VOLT, &batteryVoltageBuffer10[0], ZCV,
                                              &bat10_sum, battery10Index);
	} else {
		BMT_status.bat_vol =
		    mt_battery_average_method(BATTERY_AVG_VOLT, &batteryVoltageBuffer[0], bat_vol,
					      &bat_sum, batteryIndex);
		Avg10_bat_vol =
                    mt_battery_average10_method(BATTERY_AVG_VOLT, &batteryVoltageBuffer10[0], bat_vol,
                                              &bat10_sum, battery10Index);
	}
	...
	if (batteryIndex >= BATTERY_AVERAGE_SIZE)
		batteryIndex = 0;
	if (battery10Index >= 10)
                battery10Index = 0;

	if (g_battery_soc_ready == KAL_FALSE)
		g_battery_soc_ready = KAL_TRUE;
#if defined(CONFIG_Z380M)
	BMT_status.cover_SOC = get_cover_soc();
	get_pack_bat_vol(&pack_vol);
	battery_log(BAT_LOG_CRTI,
		    "AvgVbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),UI-SOC=(%d),ZCV=(%d),cover_SOC=(%d),cover_vol=(%d)\n",
		    BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		    BMT_status.charger_vol, BMT_status.temperature, temperature,
		    previous_SOC, BMT_status.SOC, BMT_status.UI_SOC, BMT_status.ZCV, BMT_status.cover_SOC,pack_vol);
#else
	battery_log(BAT_LOG_CRTI,
		    "AvgVbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),UI-SOC=(%d),ZCV=(%d)\n",
		    BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		    BMT_status.charger_vol, BMT_status.temperature, temperature,
...
{
	static signed int battery_temperature;
	battery_temperature = battery_meter_get_battery_temperature();
		...
		if (battery_temperature >= 60) {
...
#else
			{
				if ((g_platform_boot_mode == META_BOOT)
					...
					battery_log(BAT_LOG_CRTI,"[Battery] Tbat(%d)>=60, system need power down.\n",battery_temperature);
...
*/
...
#endif
	{
		...
		BMT_status.charger_type = CHR_Type_num;
#ifdef ENG_BUILD
		//cdp go to sdp setting for eng
		if(BMT_status.charger_type == CHARGING_HOST)
			BMT_status.charger_type = STANDARD_HOST;
#endif
#if defined(CONFIG_Z300M)
		if ((IS_STAND_AC()) && (!VBUS_IN())){//if stand AC charging, consider as AC Charging
                        BMT_status.charger_type = STAND_AC;
                }
#endif
	}
...
//		battery_charging_control(CHARGING_CMD_SET_CHRIND_CK_PDN, &pwr);
		...
		battery_log(BAT_LOG_CRTI, "[%s] wake_unlock\n",__func__);
...
static void firstbat_report_function(struct work_struct *work)
{
	battery_log(BAT_LOG_CRTI," [%s]\n", __func__);
	mt_usb_disconnect();
}
#if defined(CONFIG_Z300M)
extern void call_stand_interupt(void);
#endif

void do_chrdet_first_task(void)
{
	BMT_status.old_charging_mode = CINIT;
       if (g_bat_init_flag == KAL_TRUE) {
               if (upmu_is_chr_det() == KAL_TRUE || VBUS_IN()) {
                       battery_log(BAT_LOG_CRTI, "[%s] charger exist!\n", __func__);
#if defined(CONFIG_Z300M)
			if(VBUS_IN()){
				do_chrdet_int_task();
			}
				...
				chr_wake_up_for_meter_charging = 1;
				chr_wake_up_for_meter_discharging = 0;
			}
			return;
#endif
                       BMT_status.charger_exist = KAL_TRUE;
			...
			chr_wake_up_for_meter_charging = 0;
			chr_wake_up_for_meter_discharging = 1;
			chr_wake_up_for_meter_check = 1;
                       battery_log(BAT_LOG_CRTI, "[%s], usb in, charger type = %d\n", __func__, BMT_status.charger_type);
#if defined(CONFIG_Z380M)
                       if (COVER_ATTACHED()){
                               battery_log(BAT_LOG_CRTI, "[%s] COVER DET\n", __func__);
                               if (BMT_status.charger_type == STANDARD_HOST || (BMT_status.charger_type == CHARGING_HOST) || (BMT_status.charger_type == STANDARD_CHARGER)){
                                       battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD + COVER + USB]\n", __func__);
					call_cover_interupt();
                               }
				else if (BMT_status.charger_type == NONSTANDARD_CHARGER){
					if(!VBUS_IN()){
						battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD + COVER] \n", __func__);
						call_cover_interupt();
					}
					else{
						battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD + COVER + xAC] \n", __func__);
						call_cover_interupt();
					}
				}
                       }
			...
			{
                               if (BMT_status.charger_type == STANDARD_HOST){
                                       battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD + SDP] \n", __func__);
                                       battery_charging_control(CHARGING_CMD_SDP, NULL);
				}
                               else if (BMT_status.charger_type == CHARGING_HOST){
					battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD + CDP] \n", __func__);
					battery_charging_control(CHARGING_CMD_CDP, NULL);
                               }else if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||(BMT_status.charger_type == STANDARD_CHARGER)){
                                       battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD + AC] \n", __func__);
                                       battery_charging_control(CHARGING_CMD_AC, NULL);
                               }
                       }
               } else {
                       battery_log(BAT_LOG_CRTI, "[%s] charger NOT exist!\n", __func__);
			chr_wake_up_for_meter_charging = 1;
			chr_wake_up_for_meter_discharging = 0;
                       BMT_status.charger_exist = KAL_FALSE;
#if defined(CONFIG_Z380M)
...
#endif
                       {
                               battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD only] \n", __func__);
                               battery_charging_control( CHARGING_CMD_INIT, NULL);
                       }
#ifdef CONFIG_MTK_KERNEL_POWER_OFF_CHARGING
                       if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
                               || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
                               battery_log(BAT_LOG_CRTI,"[pmic_thread_kthread] Unplug Charger/USB In Kernel Power Off Charging Mode!  Shutdown OS!\r\n");
                               battery_charging_control(CHARGING_CMD_SET_POWER_OFF, NULL);
                       }
#endif
//                       wake_unlock(&battery_suspend_lock);
		...
		if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||(BMT_status.charger_type == STANDARD_CHARGER)){
			if(strcmp("z380m_kd_rm68200g01", lk_lcmname)){
				eKTH3260_cable_status_handler(1);
			}else{
				gt6108_cable_status_handler(1);
			}
		}else if ((BMT_status.charger_type == STANDARD_HOST) || (BMT_status.charger_type == CHARGING_HOST)){
			if(strcmp("z380m_kd_rm68200g01", lk_lcmname)){
                                eKTH3260_cable_status_handler(2);
                        }else{
                                gt6108_cable_status_handler(2);
                        }
		}else if(COVER_ATTACHED()){
			upi_ug31xx_attach(true);
//			upi_ug31xx_alarm_callback();
			if(strcmp("z380m_kd_rm68200g01", lk_lcmname)){
                                eKTH3260_cable_status_handler(3);
                        }else{
			...
			if(strcmp("z380m_kd_rm68200g01", lk_lcmname)){
                                eKTH3260_cable_status_handler(0);
                        }else{
               ...
               battery_log(BAT_LOG_CRTI,"[%s] battery thread not ready, will do after bettery init.\n", __func__);
       }
}
...
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if (g_otg_enable)
			battery_log(BAT_LOG_CRTI,"[%s] OTG mode, do not thing here.\n",__func__);
#if defined(CONFIG_Z380M)
		/* cb81 charging to pad, triger vcdt interrupt */
		else if (g_cb81_int){
			battery_log(BAT_LOG_CRTI,"[%s] cb81 mode, do not thing here.\n",__func__);
			g_cb81_int = 0;
		}
#elif defined(CONFIG_Z300M)
		else if (g_stand_int){
			battery_log(BAT_LOG_CRTI,"[%s] stand(AC) mode, do not thing here.\n",__func__);
			g_stand_int = 0;
		}
#endif
		else if (upmu_is_chr_det() == KAL_TRUE || VBUS_IN()) {
#else
...
#endif
			if(!g_projector_on)
				msleep_interruptible(1000); //delay 1s
			battery_log(BAT_LOG_CRTI, "[%s] charger exist!\n", __func__);
			battery_log(BAT_LOG_CRTI, "[%s] charger type = %d\n", __func__, BMT_status.charger_type);
			BMT_status.charger_exist = KAL_TRUE;
...
//			}
			...
			if(!BMT_status.charger_type && !g_projector_on)
				error_type = 1;
			
			battery_log(BAT_LOG_CRTI, "[%s], usb interrupt, charger type = %d, error_type = %d, VBUS = %d\n",__func__, BMT_status.charger_type, error_type, VBUS_IN());
#if defined(CONFIG_Z380M)
			if (COVER_ATTACHED()){
				 battery_log(BAT_LOG_CRTI, "[%s] COVER DET\n", __func__);
				if ((BMT_status.charger_type == STANDARD_HOST) || (BMT_status.charger_type == CHARGING_HOST) || (BMT_status.charger_type == STANDARD_CHARGER)){
					...
					if (BMT_status.old_charging_mode == PAD){
						...
						if (usb_in_int){ // workaround: cb81+pad boot will come to this function
							battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD + COVER + AC] \n", __func__);
							battery_charging_control(CHARGING_CMD_COVER_AC, NULL);
						}
					}
				}
				else if (BMT_status.old_charging_mode == COVER_SDP && error_type){
					battery_log(BAT_LOG_CRTI,"[shliun] sdp workaround.\n");
					call_cover_interupt();
				}
				else if (BMT_status.old_charging_mode == COVER && error_type)
				{
					battery_log(BAT_LOG_CRTI,"[shilun] Charging mode: [PAD + COVER + SDP] \n");
//					battery_charging_control(CHARGING_CMD_COVER_SDP, NULL);
					call_cover_interupt();
				}
				else if(_IS_CA81_())
				{
					battery_log(BAT_LOG_CRTI,"[shliun] ca81 setting\n");
					call_cover_interupt();
				}
				else
					battery_log(BAT_LOG_CRTI, "[shilun] error_type = %d, charging mode = %d, charger type = %d, VBUS = %d\n", error_type, BMT_status.old_charging_mode, BMT_status.charger_type, VBUS_IN());
				g_cvbus_in_int = 0;
			}
			else 
#endif
...
#if defined(CONFIG_Z380M)
				else if (BMT_status.charger_type == 0){
					if(VBUS_IN())
					{
						battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [sdp] workaround\n", __func__);
						battery_charging_control(CHARGING_CMD_SDP, NULL);
					}
					else
					{
						battery_log(BAT_LOG_CRTI,"[%s]Charging mode: [PAD only] workaround\n", __func__);
						battery_charging_control(CHARGING_CMD_INIT, NULL);
					}
				}
#endif
			...
			BMT_status.charger_type = CHARGER_UNKNOWN;
				...
				battery_log(BAT_LOG_CRTI,"[%s] Charging mode: [PAD + COVER] \n", __func__);
				call_cover_interupt();
//				battery_charging_control( CHARGING_CMD_COVER, NULL);
			}
			else
#endif
			{
				if(g_otg_enable){
					battery_log(BAT_LOG_CRTI,"[%s] Charging mode: [OTG + stand(AC)/CB81] \n", __func__);
				}
				else{
					battery_log(BAT_LOG_CRTI,"[%s] Charging mode: [PAD only] \n", __func__);
					battery_charging_control( CHARGING_CMD_INIT, NULL);
...
#endif
			wake_unlock(&battery_suspend_lock);
                        battery_log(BAT_LOG_CRTI,"[%s] battery_suspend_unlock\n",__func__);
#ifdef CONFIG_MTK_KERNEL_POWER_OFF_CHARGING
			if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
			    || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
				if(b_COS_CO == false){
				...
				/* mt_power_off(); */
					...
					wake_lock_timeout(&battery_COS_CO_lock, HZ*8);
					battery_log(BAT_LOG_CRTI,"[%s] wake_lock_timeout!!\n",__func__);
				}
			}
#endif
...
//			}
	...
	if(upmu_is_chr_det() == KAL_TRUE && chr_wake_up_for_meter_charging == 1){//Plug_In		
                chr_wake_up_for_meter_charging = 0;
                chr_wake_up_for_meter_discharging = 1;
		chr_wake_up_for_meter_check = 1;
...
#if defined(CONFIG_Z380M)
                        if (COVER_ATTACHED()){
                               battery_log(BAT_LOG_FULL,"[%s]Cover,skip_battery_update\n",__func__);
                               skip_battery_update = KAL_TRUE;
                       }
#endif
                        mt_battery_update_status();
		...
		wake_up_bat();
	}else if(upmu_is_chr_det() == KAL_FALSE && chr_wake_up_for_meter_discharging == 1){//Plug_Out
                chr_wake_up_for_meter_charging = 1;
                chr_wake_up_for_meter_discharging = 0;
		chr_wake_up_for_meter_check = 1;
...
#elif defined(CONFIG_Z300M)
		if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||(BMT_status.charger_type == STANDARD_CHARGER))
			ft5726_cable_status_handler(1);
		else if ((BMT_status.charger_type == STANDARD_HOST) || (BMT_status.charger_type == CHARGING_HOST))
			ft5726_cable_status_handler(2);
		else if (IS_STAND_AC())
			ft5726_cable_status_handler(3);
		else
			ft5726_cable_status_handler(0);
#endif
	} else {
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
...
void BAT_thread(void)
{
	...
	/*Clear bq25896 watchdog without charging*/
	if (chargin_hw_init_done){
                battery_charging_control(CHARGING_CMD_RESET_WATCH_DOG_TIMER, NULL);
		battery_log(BAT_LOG_CRTI, "[%s]Clear bq25896 watchdog\n",__func__);
	}
	/******************************/
	...
	mt_battery_charger_detect_check();
//	battery_log(BAT_LOG_CRTI, "[%s]after mt_battery_charger_detect_check\n",__func__);
	mt_battery_GetBatteryData();
//	battery_log(BAT_LOG_CRTI, "[%s]after mt_battery_GetBatteryData\n",__func__);
	if (BMT_status.charger_exist == KAL_TRUE)
		check_battery_exist();
...
//		battery_log(BAT_LOG_CRTI, "BMT_status.bat_charging_state = %d\n",BMT_status.bat_charging_state);
...
/*		battery_log(BAT_LOG_CRTI, "[%s]chargin_hw_init_done=%d, battery_suspended=%d, fg_wake_up_bat=%d\n",
			__func__,chargin_hw_init_done,battery_suspended,fg_wake_up_bat);*/
		if (((chargin_hw_init_done == KAL_TRUE) && (battery_suspended == KAL_FALSE))
		    || ((chargin_hw_init_done == KAL_TRUE) && (fg_wake_up_bat == KAL_TRUE))){
			BAT_thread();
			battery_log(BAT_LOG_CRTI,"[%s] after BAT_thread\n",__func__);
		}
		...
		battery_log(BAT_LOG_CRTI, "wait event\n");
		wait_event(bat_thread_wq, (bat_thread_timeout == KAL_TRUE));
		battery_log(BAT_LOG_CRTI,"[%s] after wait_event\n",__func__);
		bat_thread_timeout = KAL_FALSE;
		hrtimer_start(&battery_kthread_timer, ktime, HRTIMER_MODE_REL);

		if (BMT_status.SOC >= 50){
			BAT_TASK_PERIOD=60;
		}else if((BMT_status.SOC < 50) && (BMT_status.SOC > 10)){
			BAT_TASK_PERIOD=30;
		}else if(BMT_status.SOC <= 10){
			BAT_TASK_PERIOD=10;
		}
...
static void atimer_wq_function(struct work_struct *work){

	if (upmu_is_chr_det() == KAL_TRUE || VBUS_IN()) {//Charging?
//                battery_log(BAT_LOG_CRTI," [%s] Charging = TRUE\n",__func__);
                if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||(BMT_status.charger_type == STANDARD_CHARGER)){//AC?
#if defined(CONFIG_Z380M)
			if(COVER_ATTACHED())
				cover_ac_function_for_alarm();//Add for Back Cover switch charging algorithm
#endif
                        battery_charging_control(CHARGING_CMD_JEITA, NULL);
                        battery_log(BAT_LOG_CRTI," [%s] JEITA Complete!!\n",__func__);
                }
	}
        wake_unlock(&battery_suspend_lock);
	battery_log(BAT_LOG_CRTI," [%s] wake_unlock\n",__func__);
...
static enum alarmtimer_restart Wake_BAT_polling_atimer(struct alarm *alrm, ktime_t t)
{
	wake_lock(&battery_suspend_lock);
	battery_log(BAT_LOG_CRTI," [%s] Start!!\n",__func__);

	if(atomic_read(&Wake_BAT_alarm) == 1){
	        queue_delayed_work(atimer_wq, &atimer_work, msecs_to_jiffies(0));
		alarm_forward_now(&Wake_BAT_alarm_timer, ms_to_ktime(60000));//Need to  set next restart time
		battery_log(BAT_LOG_CRTI," [%s] Set Wake_BAT_alarm_timer = 60\n",__func__);
		_g_bat_sleep_total_time = 0;
		battery_log(BAT_LOG_CRTI," [%s] Reset _g_bat_sleep_total_time!!\n",__func__);
...
static int battery_probe(struct platform_device *dev)
{
	...
	wake_lock_init(&battery_COS_CO_lock, WAKE_LOCK_SUSPEND, "battery cos cable out wkaelock");
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
	wake_lock_init(&TA_charger_suspend_lock, WAKE_LOCK_SUSPEND, "TA charger suspend wakelock");
#endif
	atimer_wq = create_singlethread_workqueue("atimer_wq");
	INIT_DEFERRABLE_WORK(&atimer_work, atimer_wq_function);
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_COS_CO);
	...
	if ((g_vcdt_irq_delay_flag == KAL_TRUE) || (upmu_is_chr_det() == KAL_TRUE))
		do_chrdet_int_task();
#endif
	alarm_init(&Wake_BAT_alarm_timer, ALARM_REALTIME, Wake_BAT_polling_atimer);
	firstbat_wq = create_singlethread_workqueue("firstbat_wq");
	INIT_DEFERRABLE_WORK(&firstbat_work, firstbat_report_function);
	return 0;
}
...
//	hvtime = ktime_set(0, BAT_MS_TO_NS(2000));
	...
	/*Add by Michael_Nieh@asus.com*/
	/*Interrupt frequently, do BAT_thread by check sleep_time.*/
	if (is_pcm_timer_trigger == KAL_TRUE || bat_spm_timeout	|| _g_bat_sleep_short_total_time >= 60) {
		mutex_lock(&bat_mutex);
		BAT_thread();
		mutex_unlock(&bat_mutex);
	} else {
	...
	/*USB charging wont suspend; AC charging will suspend*/
	if(BMT_status.charger_exist == KAL_TRUE){
		_g_bat_sleep_short_total_time += (bat_time_after_sleep.tv_sec - g_bat_time_before_sleep.tv_sec);
	}
...
static int battery_pm_prepare(struct device *device)
{
        int ret = 0;
...
#if defined(CONFIG_Z300M)
                if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||(BMT_status.charger_type == STANDARD_CHARGER)){//AC?
#elif defined(CONFIG_Z380M)
                if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||(BMT_status.charger_type == STANDARD_CHARGER)|| (_IS_CB81_()&&!VBUS_IN())){//AC?
#endif
                        battery_log(BAT_LOG_CRTI," [%s] Charging Type= AC\n",__func__);
                        if (atomic_read(&Wake_BAT_alarm) == 0) {
				alarm_start_relative(&Wake_BAT_alarm_timer, ms_to_ktime(60000));
				battery_log(BAT_LOG_CRTI," [%s] Set Wake_BAT_alarm_timer = 60\n",__func__);
				atomic_set(&Wake_BAT_alarm,1);
			}
                }
        }
        return ret;
...
static int __init battery_init(void)
{
	...
	do_chrdet_first_task();
	battery_log(BAT_LOG_CRTI, "****[battery_driver] [do_chrdet_first_task] for init current !!! ****\n");
