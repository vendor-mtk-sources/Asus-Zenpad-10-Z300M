...
//Andy add ++
static int __init check_slimport_id(char *p){
        if (p) {
                sscanf(p, "%d", &slimport_id);
                printk(KERN_ERR "%s:slimport_id = %d\n", __func__, slimport_id);
        }
        return 0;
}
early_param("slimport_id", check_slimport_id);

module_param(slimport_id, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(SLIMPORT_VERSION, "SLIMPORT_ID judgement");
int Read_SLIMPORT_ID(void)
{
        printk(KERN_INFO "SLIMPORT_ID = 0x%x \n", slimport_id);
        return slimport_id;
}
EXPORT_SYMBOL(Read_SLIMPORT_ID);
//Andy add --

//Jui add for detect OS entry mode ++
...
//Mini-porting ++
    ...
    loc_reco = strstr(boot_command_line,"androidboot.mode=recovery");
    if (loc_reco != NULL) {
            entry_mode = 2;
